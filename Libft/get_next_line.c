...
/*   Updated: 2021/10/07 10:21:16 by dmylonas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*setter_without(char *line, char *holder)
{
	...
	new_line = gnl_ft_strjoin(line, holder);
	free(line);
	line = NULL;
	holder[0] = '\0';
	return (new_line);
...
char	*setter_with(char *line, char *holder)
{
	...
	temp = gnl_ft_substr(holder, based_on_ft_strchr(holder),
...
char	*get_next_line(int fd)
{
	...
	line = (char *) malloc(1);
	line[0] = 0;
	while (!based_on_ft_strchr(holder))
	{
		if (holder[0])
